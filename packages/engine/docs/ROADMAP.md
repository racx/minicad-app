...
| AREA | `AREA` `AA` | ✅ Click a hatch or closed shape → area + perimeter (shoelace + circular-segment corrections, bulge-aware — `geometry.js` plineArea/entityArea). Falls back to the boundary tracer on loose linework — reports the enclosed region's figure without creating anything. | suites 22+39 |
